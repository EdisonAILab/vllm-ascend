...
_GROUPED_BI_GRAPH_NATIVE = os.environ.get("VLLM_MXFP8_GROUPED_BI_GRAPH_NATIVE") == "1"
...
def _dequant_weight(
    ...
    if scale.ndim == 2:
        ...
        raise RuntimeError(f"unsupported MXFP8 weight-scale rank {scale.ndim}; expected 2 or 3")
...
def _dense_bi_matmul(
    ...
    if cached is None:
        cached = _dequant_weight(layer.weight, layer.weight_scale, group_size).to(torch.bfloat16)
        layer._bi_dense_bf16_weight = cached

    x_bf16 = _dequant_activation(quantized_x, pertoken_scale, group_size).to(torch.bfloat16)
...
def _group_offsets(
    ...
    cumulative = bool((groups[1:] >= groups[:-1]).all().item()) and int(groups[-1].item()) == total
    if not cumulative:
...
def _grouped_weight_bf16(
    ...
    if cached is None:
        cached = _dequant_weight(weight[expert], packed_scale[expert], group_size).to(torch.bfloat16)
...
def _grouped_weights_bf16_native(
    ...
    for expert in range(weight.shape[0]):
        converted[expert] = _dequant_weight(weight[expert], packed_scale[expert], group_size).to(torch.bfloat16)
...
def _grouped_gmm2_graph_bi(
    *,
    ...
    values_bf16 = _dequant_activation(values, token_scale, group_size=32).to(torch.bfloat16)
...
def _grouped_gmm1_graph_bi(
    *,
    ...
    values_bf16 = _dequant_activation(x, x_scale, group_size=32).to(torch.bfloat16)
    weights_bf16 = _grouped_weights_bf16_native(weights, scales, 32)
...
def _grouped_gmm2_bi(
    ...
    **kwargs,
    ...
    token_scale = per_token_scale[0] if isinstance(per_token_scale, (list, tuple)) else per_token_scale
    ...
    for expert in range(experts):
        ...
        if end <= start:
            continue
        expert_weight = _grouped_weight_bf16(weights, scales, expert, 32, "gmm2")
        part = torch.ops.batch_invariant_ops.npu_mm_batch_invariant(
            values_bf16[start:end].contiguous(),
...
def _grouped_gmm1_bi(
    ...
    **kwargs,
    ...
    del kwargs
    weights = weight[0] if isinstance(weight, (list, tuple)) else weight
    scales = weight_scale[0] if isinstance(weight_scale, (list, tuple)) else weight_scale
    if _GROUPED_BI_GRAPH_NATIVE:
    ...
    for expert in range(experts):
        ...
        if end <= start:
            continue
        expert_weight = _grouped_weight_bf16(weights, scales, expert, 32, "gmm1")

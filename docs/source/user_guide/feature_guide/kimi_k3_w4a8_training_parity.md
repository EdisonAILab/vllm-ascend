...
This branch contains an opt-in correctness path for localizing numerical
...
The path is disabled by default. It is a correctness reference for eager TP1
execution, not a serving-performance mode. The validated reduced model has 8
layers, hidden size 1,024, 6 KDA layers, 2 MLA layers, and top-2 routing over 8
experts.
...
export VLLM_ASCEND_KIMI_REFERENCE_MXFP8_DISPATCH=1
export VLLM_ASCEND_KIMI_REFERENCE_ROUTER_WEIGHT_BEFORE_GMM2=1
...
contract. The reference MXFP8 dispatch boundary quantizes the sorted BF16
tokens with `npu_dynamic_mx_quant`, and routing weights are applied before the
second dynamic MXFP8 quantization and grouped GEMM.
